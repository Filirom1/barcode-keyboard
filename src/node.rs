...
impl Echo {
    ...
    async fn handle_connection_0(
        ...
        // Read all bytes then echo them back. Use the quinn-native read_to_end
        // (takes a max byte limit) to avoid name conflict with AsyncReadExt.
        let bytes = recv
            .read_to_end(1_000_000)
            .await
            .map_err(|e| std::io::Error::other(e))?;
        let content = String::from_utf8_lossy(&bytes).into_owned();
        info!("Received {} byte(s): {:?}", bytes.len(), content);

        send.write_all(&bytes)
            .await
            .map_err(|e| std::io::Error::other(e))?;
        self.event_sender
            .send(AcceptEvent::Received { endpoint_id, content })
            .ok();
